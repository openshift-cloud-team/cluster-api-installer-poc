...
- **`infrastructureRef` is a `ContractVersionedObjectReference` in v1beta2, not an
  ...
  to resolve. Install with `make deploy`.
...
5. **The baremetal bootstrap ignition carries Ironic/Metal3, and it will deadlock.** This is the
   ...
   gated on the MachineAPI capability. So `bootstrap.ign` always includes
   `build-ironic-env.service`, `build-metal3-env.service`, `master-bmh-update.service`,
   `provisioning-interface.service` and `extract-machine-os.service`. `master-bmh-update.sh` opens
   with `until oc get baremetalhosts -n openshift-machine-api` — with MachineAPI disabled that CRD
   never exists, so the loop never exits. And the script's own comment gives the consequence: the
   step after that wait is *"Shut down ironic containers so that the API VIP can fail over to the
   control plane."* **That unit is what hands the API VIP from the bootstrap node to the masters.**
   Left in place it hangs forever, the bootstrap node keeps the VIP, and the install cannot
   complete.
   Mitigation: `provisioningNetwork: Disabled` *and* a `jq` post-processing step in
   `create-cluster.sh` stripping those five units from `bootstrap.ign` before it is embedded —
   filtering `.systemd.units[]` by name and dropping the matching `/usr/local/bin/*.sh` entries
   from `.storage.files[]`. With ironic gone there should be no VIP contention and keepalived's
   normal priority handover should apply, but this is the assumption most in need of a real test.
...
12. **The `openshift/installer` checkout used for these citations is from 2024-08-12** and
    ...
    `onPremPlatform` list against the release actually being targeted.
...
3. Confirm the load-bearing capability claim concretely: build an install-config with
   `platform: baremetal`, MachineAPI disabled and no `hosts[]`, run
   `openshift-install create manifests`, and assert it exits zero and that
   `openshift/99_openshift-cluster-api_*` and the CPMS manifest are absent.
4. Run `create ignition-configs`, confirm the five Ironic units are present in `bootstrap.ign`,
   and confirm the `jq` filter removes them and their backing scripts cleanly.
